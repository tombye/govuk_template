...
require_relative '../compiler/gsp_processor'

module Packager
  class GSPPackager < TarPackager
    def initialize
      super
      @base_name = "gsp_govuk_template-#{GovukTemplate::VERSION}"
    end
    ...
    def process_template(file)
      ...
      File.open(target_dir.join(generated_file_name(file)), 'wb') do |f|
        f.write Compiler::GSPProcessor.new(file).process
      end
    end
